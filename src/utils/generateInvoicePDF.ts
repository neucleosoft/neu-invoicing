...
function drawLogo(doc: jsPDF, x: number, y: number, width: number, height: number, logoBase64?: string) {
  try {
...
function generateClassicTemplate(doc: jsPDF, invoice: InvoiceData) {
  ...
  drawLogo(doc, ML + 2, BORDER_TOP + 2, LOGO_SIZE, LOGO_SIZE, companyLogo)
  ...
  drawLogo(doc, sigCenterX - 6, FOOTER_TOP + 5, 12, 12, companyLogo)
  doc.setFontSize(7.5)
  doc.setFont('helvetica', 'normal')
