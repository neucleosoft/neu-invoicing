// Delivery Challan pdfmake document-definition builder. Lifted from
// apps/desktop/src/utils/pdfmakeChallan.ts — the EXACT same layout — minus the
...
//
// Logo: rendered only when ch.company.logoBase64 is present (a base64 data-URI).
...
export function buildChallanFilename(challan: ChallanData): string {
  return `${challan.challanNumber.replace(/\//g, '_')}_delivery_challan_${challan.customer.name.replace(/[^a-z0-9]/gi, '_')}.pdf`
}

export function buildChallanDocDefinition(ch: ChallanData): DocDefinition {
  ...
  const gridBody: TableCell[][] = [
    [
      { stack: [
        ...
        { text: ch.challanNumber, fontSize: 10, margin: [0, 3, 0, 0] },
      ] },
      { stack: [
        { text: 'Challan Date', bold: true, fontSize: 10 },
        { text: formatDate(ch.challanDate), fontSize: 10, margin: [0, 3, 0, 0] },
      ] },
    ],
    [
      { text: '' },
      { stack: [
        ...
        { text: statusLabel, fontSize: 10, margin: [0, 3, 0, 0] },
      ] },
    ],
  ]
...
function buildItemsSection(ch: ChallanData, isInter: boolean, taxGroups: ReturnType<typeof getTaxGroups>): Content {
  ...
  // Whole-rupee Round Off (see invoice.ts — same guard: only sub-50-paise deltas).
  ...
  const totalRow: TableCell[] = [
    ...
    { text: '', fillColor: GREEN },
    { text: fmtRs(ch.totalAmount), bold: true, fontSize: 9, alignment: 'right', fillColor: GREEN },
  ]
      ...
      body: [[
        {
          stack: [
            ...
            { text: words, fontSize: 9, margin: [0, 2, 0, 0] },
          ],
          fillColor: GREEN,
        }
      ]],
...
function buildFooter(ch: ChallanData, logo: string | undefined): Content {
  const company = ch.company
