// Customer-statement / ledger pdfmake document-definition builder. Lifted from
// apps/desktop/src/utils/pdfmakeStatement.ts — the EXACT same layout — minus the
...
//
// This one builder powers the customer statement AND the customer/supplier
// ledgers — they differ only by the `title` field on the input.
//
// Logo: rendered only when company.logoBase64 is present (a base64 data-URI).
...
import { fmtBalanceNum, fmtBalanceRs, fmtNum, formatDate } from './helpers'
...
export function buildStatementDocDefinition(data: StatementData): DocDefinition {
  // Only embed the logo when it's a data: URI (the actual image bytes). A stale
  // file path or the old corrupt LOGO_BASE64 fallback would make pdfmake throw.
  // Anything that isn't a data: URI is treated as "no logo".
  const logoSrc = data.company?.logoBase64
...
function buildHeader(data: StatementData, logo: string | undefined): Content {
