...
export async function recomputeAll(
  ...
  const [customers, suppliers, invoices, bills, payments, notes] = await Promise.all([
    db.select({ id: schema.customer.id, name: schema.customer.name, currentBalance: schema.customer.currentBalance, openingBalance: schema.customer.openingBalance }).from(schema.customer),
    db.select({ id: schema.supplier.id, name: schema.supplier.name, currentBalance: schema.supplier.currentBalance, openingBalance: schema.supplier.openingBalance }).from(schema.supplier),
    db.select({ id: schema.salesInvoice.id, invoiceNumber: schema.salesInvoice.invoiceNumber, customerId: schema.salesInvoice.customerId, totalAmount: schema.salesInvoice.totalAmount, amountPaid: schema.salesInvoice.amountPaid, balanceDue: schema.salesInvoice.balanceDue, status: schema.salesInvoice.status, deletedAt: schema.salesInvoice.deletedAt, cancelledAt: schema.salesInvoice.cancelledAt }).from(schema.salesInvoice).where(eq(schema.salesInvoice.type, 'INVOICE')),
  ...
  try {
    ;[items, movements] = await Promise.all([
      db.select({ id: schema.item.id, name: schema.item.name, currentStock: schema.item.currentStock, openingStock: schema.item.openingStock }).from(schema.item),
  ...
  if (apply && totalChanges > 0) {
    await db.transaction(async (tx) => {
      for (const c of customers) {
        ...
        if (Math.abs(v - c.currentBalance) > EPS) await tx.update(schema.customer).set({ currentBalance: round2(v) }).where(eq(schema.customer.id, c.id))
      }
      for (const s of suppliers) {
        const v = supBal.get(s.id) ?? 0
        if (Math.abs(v - s.currentBalance) > EPS) await tx.update(schema.supplier).set({ currentBalance: round2(v) }).where(eq(schema.supplier.id, s.id))
      }
      for (const i of invoices) {
        ...
        if (moneyMoved || statusMoved) await tx.update(schema.salesInvoice).set({ amountPaid: round2(st.amountPaid), balanceDue: round2(st.balanceDue), status: st.status }).where(eq(schema.salesInvoice.id, i.id))
      }
      for (const b of bills) {
        ...
        if (moneyMoved || st.status !== b.status) await tx.update(schema.purchaseBill).set({ amountPaid: round2(st.amountPaid), balanceDue: round2(st.balanceDue), status: st.status }).where(eq(schema.purchaseBill.id, b.id))
      }
      if (stockAvailable) {
        for (const it of items) {
          const v = stock.get(it.id) ?? 0
          if (Math.abs(v - it.currentStock) > EPS) await tx.update(schema.item).set({ currentStock: round2(v) }).where(eq(schema.item.id, it.id))
        }
      }
    })
  }
