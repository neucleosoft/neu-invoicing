...
// Same 8 options desktop Items.tsx:327-334 exposes.
...
export default function EditItemScreen() {
  ...
  const [originalItem, setOriginalItem] =
    useState<typeof schema.item.$inferSelect | null>(null)
  ...
  useEffect(() => {
    ...
    db.select()
      ...
      .then((rows) => {
        ...
        setName(it.name)
        setSkuHsn(it.skuHsn ?? '')
        setType(it.type === 'SERVICE' ? 'SERVICE' : 'PRODUCT')
        // If the legacy unit isn't one of the 8 desktop options (e.g. an old
        // free-text value like "Box "), fall back to pcs so the picker isn't
        // stuck on an invalid option.
        const allowedUnits = UNIT_OPTIONS as readonly string[]
        setUnit(
          allowedUnits.includes(it.unit) ? (it.unit as UnitOption) : 'pcs',
        )
        setSalePrice(String(it.salePrice))
        setPurchasePrice(String(it.purchasePrice))
        setTaxRate(String(it.taxRate))
        setTrackStock(it.trackStock)
        setCurrentStock(String(it.currentStock))
        setLowStockWarning(String(it.lowStockWarning))
        setLoading(false)
      })
  }, [id, db])
