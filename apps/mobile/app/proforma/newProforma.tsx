...
// rules as a Quotation (no balance/stock on save; convertible to a real
// invoice). Only the table, number series (NS/PI) and labels differ.
...
export default function NewProformaScreen() {
  const db = useDb()
  ...
  function updateLine(index: number, patch: Partial<LineRow>) { setLines(lines.map((l, i) => (i === index ? { ...l, ...patch } : l))) }
  ...
  async function handleSave() {
    ...
    if (!dDate) { Alert.alert('Validation', 'Invalid date (use YYYY-MM-DD)'); return }
    setSaving(true)
    try {
      ...
      Alert.alert('Error', e instanceof Error ? e.message : 'Failed to save proforma')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <ThemedText type="title" style={styles.title}>New Proforma</ThemedText>

      <ThemedText style={styles.label}>Proforma #</ThemedText>
      <ThemedView style={styles.readOnly}><ThemedText>{docNumber}</ThemedText></ThemedView>

      <ThemedText style={styles.label}>Customer *</ThemedText>
      <Pressable style={styles.picker} onPress={() => setShowCustomerPicker(true)}>
        <ThemedText style={selectedCustomer ? undefined : styles.placeholder}>{selectedCustomer ? selectedCustomer.name : 'Tap to select customer'}</ThemedText>
      </Pressable>
      ...
      <Pressable style={[styles.saveButton, saving && styles.saveButtonDisabled]} onPress={handleSave} disabled={saving}>
        <ThemedText style={styles.saveButtonText}>{saving ? 'Saving…' : 'Save Proforma'}</ThemedText>
      </Pressable>
      ...
      <Modal visible={showItemPicker} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <ThemedView style={styles.modalContent}>
            ...
            <FlatList data={items} keyExtractor={(it) => it.id} ListEmptyComponent={<ThemedText style={styles.modalEmpty}>No items yet.</ThemedText>} renderItem={({ item }) => (
    ...
    <Modal visible={visible} animationType="slide" transparent>
      <View style={styles.modalOverlay}>
        <ThemedView style={styles.modalContent}>
          ...
          <FlatList data={data} keyExtractor={(o) => o.key} ListEmptyComponent={<ThemedText style={styles.modalEmpty}>Nothing here yet.</ThemedText>} renderItem={({ item }) => (
            <Pressable style={styles.modalRow} onPress={() => { onSelect(item.key); onClose() }}>
              <ThemedText type={item.key === selectedKey ? 'defaultSemiBold' : undefined}>{item.key === selectedKey ? `✓ ${item.label}` : item.label}</ThemedText>
            </Pressable>
          )} />
          <Pressable style={styles.modalClose} onPress={onClose}><ThemedText style={styles.modalCloseText}>Cancel</ThemedText></Pressable>
        </ThemedView>
      </View>
    </Modal>
  )
