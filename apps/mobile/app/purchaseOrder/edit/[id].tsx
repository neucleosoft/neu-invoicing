...
type OrderLine = { supplierItemId: string | null; name: string; hsnCode: string; qty: number; rate: number; discount: number; taxRate: number }

const lineAmount = (l: OrderLine) => (l.qty * l.rate - l.discount) * (1 + l.taxRate / 100)
...
export default function EditPurchaseOrderScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      const [po] = await db.select().from(schema.purchaseOrder).where(eq(schema.purchaseOrder.id, id)).limit(1)
      if (!po) { setFound(false); setLoading(false); return }
      ...
      setLines(its.map((bi) => ({ supplierItemId: bi.supplierItemId, name: nameById.get(bi.supplierItemId) ?? 'Item', hsnCode: bi.hsnCode ?? '', qty: bi.quantity, rate: bi.rate, discount: bi.discount, taxRate: bi.taxRate })))
      setLoading(false)
    }
    load()
  }, [id, db])
  ...
  function pickSupplier(newId: string) { if (newId !== supplierId) setLines([]); setSupplierId(newId); setShowSupplierPicker(false) }
  function addFromCatalog(si: SupplierItem) { setLines((prev) => [...prev, { supplierItemId: si.id, name: si.name, hsnCode: si.hsnCode ?? '', qty: 1, rate: si.lastPurchasePrice, discount: 0, taxRate: si.defaultTaxRate }]); setShowCatalogPicker(false) }
  function addNewLine() { setLines((prev) => [...prev, { supplierItemId: null, name: '', hsnCode: '', qty: 1, rate: 0, discount: 0, taxRate: 0 }]) }
  function updateLine(index: number, patch: Partial<OrderLine>) { setLines((prev) => prev.map((l, i) => (i === index ? { ...l, ...patch } : l))) }
  function removeLine(index: number) { setLines((prev) => prev.filter((_, i) => i !== index)) }

  async function handleSave() {
    if (!id) return
    if (!supplierId) { Alert.alert('Validation', 'Please select a supplier'); return }
    if (lines.length === 0) { Alert.alert('Validation', 'Add at least one line item'); return }
    if (lines.some((l) => !l.supplierItemId && !l.name.trim())) { Alert.alert('Validation', 'Every new line needs an item name'); return }
    setSaving(true)
    try {
  ...
  if (!found) return <ThemedView style={styles.loadingContainer}><ThemedText type="subtitle">Purchase order not found</ThemedText><Pressable style={styles.backLink} onPress={() => router.back()}><ThemedText style={styles.backLinkText}>Go back</ThemedText></Pressable></ThemedView>

  return (
    <ScrollView contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      <ThemedText type="title" style={styles.title}>Edit Purchase Order</ThemedText>

      <ThemedText style={styles.label}>PO #</ThemedText>
      <View style={styles.lockedField}><ThemedText style={styles.lockedText}>{orderNumber}</ThemedText><ThemedText style={styles.lockedHint}>locked</ThemedText></View>

      <ThemedText style={styles.label}>Supplier *</ThemedText>
      <Pressable style={styles.picker} onPress={() => setShowSupplierPicker(true)}><ThemedText style={supplierId ? undefined : styles.placeholder}>{supplierId ? supplierName : 'Pick a supplier'}</ThemedText></Pressable>
        ...
        lines.map((line, index) => (
          <ThemedView key={index} lightColor="#f9fafb" darkColor="#1f2937" style={styles.lineCard}>
            <View style={styles.lineTop}>
              {line.supplierItemId ? (
                <ThemedText type="defaultSemiBold" style={styles.lineName} numberOfLines={2}>{line.name}</ThemedText>
              ) : (
                <TextInput style={[styles.input, styles.lineNameInput]} value={line.name} onChangeText={(t) => updateLine(index, { name: t })} placeholder="New item name" placeholderTextColor="#999" />
              )}
              <Pressable onPress={() => removeLine(index)} hitSlop={8} style={styles.removeBtn}><ThemedText style={styles.removeBtnText}>✕</ThemedText></Pressable>
            </View>
            ...
            <TextInput style={[styles.input, styles.hsnInput]} value={line.hsnCode} onChangeText={(t) => updateLine(index, { hsnCode: t })} placeholder="HSN / SAC (optional)" placeholderTextColor="#999" />
            <ThemedText style={styles.lineAmount}>{formatCurrency(lineAmount(line))}</ThemedText>
          </ThemedView>
        ))
      )}
      <View style={styles.addRow}>
        <Pressable style={[styles.addBtn, !supplierId && styles.addBtnDisabled]} disabled={!supplierId} onPress={() => setShowCatalogPicker(true)}><ThemedText style={styles.addBtnText}>+ From catalog</ThemedText></Pressable>
        <Pressable style={[styles.addBtn, !supplierId && styles.addBtnDisabled]} disabled={!supplierId} onPress={addNewLine}><ThemedText style={styles.addBtnText}>+ New item</ThemedText></Pressable>
      </View>
      ...
      <View style={styles.actionRow}>
        ...
        <Pressable style={[styles.saveButton, saving && styles.saveButtonDisabled]} onPress={handleSave} disabled={saving}><ThemedText style={styles.saveButtonText}>{saving ? 'Saving…' : 'Update Order'}</ThemedText></Pressable>
      </View>
      ...
      <Modal visible={showCatalogPicker} animationType="slide" transparent>
        <View style={styles.modalOverlay}><ThemedView style={styles.modalContent}>
          ...
          <Pressable style={styles.modalClose} onPress={() => setShowCatalogPicker(false)}><ThemedText style={styles.modalCloseText}>Close</ThemedText></Pressable>
        </ThemedView></View>
      </Modal>
    </ScrollView>
  )
...
function MiniField({ label, value, onChange }: { label: string; value: number; onChange: (v: number) => void }) { return (<View style={styles.miniField}><ThemedText style={styles.miniLabel}>{label}</ThemedText><TextInput style={styles.input} value={String(value)} onChangeText={(t) => onChange(parseFloat(t) || 0)} keyboardType="numeric" placeholderTextColor="#999" /></View>) }
function TotalRow({ label, value, bold }: { label: string; value: number; bold?: boolean }) { return (<View style={styles.totalRow}><ThemedText type={bold ? 'defaultSemiBold' : undefined}>{label}</ThemedText><ThemedText type={bold ? 'defaultSemiBold' : undefined}>{formatCurrency(value)}</ThemedText></View>) }
...
const styles = StyleSheet.create({
  ...
  addBtn: { flex: 1, paddingVertical: 12, borderRadius: 8, borderWidth: 1, borderColor: '#007AFF', alignItems: 'center' },
  ...
  modalRow: { paddingVertical: 14, paddingHorizontal: 8, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: '#ccc', gap: 2 },
