...
export default function EditCustomerScreen() {
  ...
  const [originalCustomer, setOriginalCustomer] =
    useState<typeof schema.customer.$inferSelect | null>(null)
  ...
  // the row and don't want the validator overwriting the saved stateCode if it
  // disagrees with the GSTIN (legacy rows may have either).
  ...
  useEffect(() => {
    ...
    db.select()
      ...
      .then((rows) => {
        ...
        setGstType(
          allowed.includes(c.gstType) ? (c.gstType as GstTypeOption) : 'REGULAR',
        )
        setBillingAddress(c.billingAddress ?? '')
        setShippingAddress(c.shippingAddress ?? '')
        setCity(c.city ?? '')
        setPincode(c.pincode ?? '')
        // Only accept the saved stateCode if it's one of the 36 known codes;
        // legacy free-text states fall back to empty so user must re-pick.
        setStateCode(
          c.stateCode && INDIAN_STATE_CODES[c.stateCode] ? c.stateCode : '',
        )
        setLoading(false)
        // Defer turning on the GSTIN→state auto-fill until after the row is
        // loaded, so the validation effect below doesn't fire during hydration.
  ...
  async function handleSave() {
    ...
    setSaving(true)
    try {
      // Mirrors desktop Customers.tsx:140-184: never touches currentBalance or
      // openingBalance on edit. Those are owned by the ledger (invoices +
      // payments + opening journal entry).
      await db
