...
export default function EditQuotationScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setQuoteNumber(q.invoiceNumber)
      const allowed = STATUS_OPTIONS as readonly string[]
      setStatus(allowed.includes(q.status) ? (q.status as StatusOption) : 'DRAFT')
      setQuoteDate(toIso(q.invoiceDate))
      setExpiryDate(toIso(q.dueDate))
      setDeliveryTime(toIso(q.deliveryTime))
      setNotes(q.notes ?? '')
      setTermsConditions(q.termsConditions ?? '')
      setDocDiscount(q.discount ?? 0)
  ...
  async function handleSave() {
    ...
    try {
      ...
      // delete-and-reinsert lines + header update, in a transaction.
      ...
      Alert.alert('Error', e instanceof Error ? e.message : 'Failed to update quotation')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <ThemedText type="title" style={styles.title}>Edit Quotation</ThemedText>

      <ThemedText style={styles.label}>Quotation #</ThemedText>
      <View style={styles.lockedField}>
        <ThemedText style={styles.lockedText}>{quoteNumber}</ThemedText>
      ...
      <View style={styles.actionRow}>
        ...
        <Pressable style={[styles.saveButton, saving && styles.saveButtonDisabled]} onPress={handleSave} disabled={saving}>
          <ThemedText style={styles.saveButtonText}>{saving ? 'Saving…' : 'Update Quotation'}</ThemedText>
        </Pressable>
      </View>
