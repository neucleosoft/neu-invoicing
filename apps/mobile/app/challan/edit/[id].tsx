...
// action and a converted challan is not editable, so it never reaches this form.
...
export default function EditChallanScreen() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setStatus(allowed.includes(dc.status) ? (dc.status as StatusOption) : 'NON_RETURNABLE')
  ...
  async function handleSave() {
    ...
    try {
      ...
      // simple delete-and-reinsert lines + header update, all in one transaction.
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <ThemedText type="title" style={styles.title}>Edit Challan</ThemedText>

      <ThemedText style={styles.label}>Challan #</ThemedText>
      <View style={styles.lockedField}>
        <ThemedText style={styles.lockedText}>{challanNumber}</ThemedText>
      ...
      <View style={styles.actionRow}>
        ...
        <Pressable style={[styles.saveButton, saving && styles.saveButtonDisabled]} onPress={handleSave} disabled={saving}>
          <ThemedText style={styles.saveButtonText}>{saving ? 'Saving…' : 'Update Challan'}</ThemedText>
        </Pressable>
      </View>
        ...
        <View style={styles.modalOverlay}>
          <ThemedView style={styles.modalContent}>
            ...
            <FlatList data={STATUS_OPTIONS} keyExtractor={(o) => o} renderItem={({ item }) => (
              <Pressable style={styles.modalRow} onPress={() => { setStatus(item); setShowStatusPicker(false) }}>
                <ThemedText type={item === status ? 'defaultSemiBold' : undefined}>{item === status ? `✓ ${item}` : item}</ThemedText>
              </Pressable>
            )} />
      ...
      <Modal visible={showTransportPicker} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <ThemedView style={styles.modalContent}>
            <ThemedText type="title" style={styles.modalTitle}>Transport Mode</ThemedText>
            <FlatList data={TRANSPORT_MODES} keyExtractor={(o) => o} renderItem={({ item }) => (
              <Pressable style={styles.modalRow} onPress={() => { setTransportMode(item); setShowTransportPicker(false) }}>
                <ThemedText type={item === transportMode ? 'defaultSemiBold' : undefined}>{item === transportMode ? `✓ ${item}` : item}</ThemedText>
              </Pressable>
            )} />
            <Pressable style={styles.modalClose} onPress={() => setShowTransportPicker(false)}><ThemedText style={styles.modalCloseText}>Cancel</ThemedText></Pressable>
          </ThemedView>
        </View>
      </Modal>
