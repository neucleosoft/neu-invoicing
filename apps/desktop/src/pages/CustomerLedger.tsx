...
import {
  buildCustomerLedger,
  buildLedgerDownloadOpts,
  getLedgerPdf,
  type LedgerData,
} from '../utils/buildCustomerLedger'
import type { Customer } from '../types'

const fmtDate = (iso: string): string =>
  new Date(iso).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })

// Show a balance without a minus sign. A negative balance means the customer
// has paid more than invoiced, i.e. they're in credit — label it "Advance".
const renderBalance = (amount: number): string =>
  amount < 0 ? `${formatCurrency(Math.abs(amount))} (Advance)` : formatCurrency(amount)

// A customer's ledger — every invoice, payment and note with a running
// balance. Data is assembled in the renderer (see buildCustomerLedger).
const CustomerLedger = () => {
  const location = useLocation()
  const navigate = useNavigate()
  const toast = useToast()
  const customer = (location.state as { customer?: Customer } | null)?.customer
  ...
  const handleShare = async (target: ShareTarget) => {
    if (!ledger) return
    try {
      const { bytes, filename } = await getLedgerPdf(ledger)
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        ...
        {ledger && (
          <div className="flex items-center gap-2">
            <DownloadMenu variant="button" getOpts={async () => buildLedgerDownloadOpts(ledger)} />
            <ShareMenu
              variant="button"
              onShare={handleShare}
