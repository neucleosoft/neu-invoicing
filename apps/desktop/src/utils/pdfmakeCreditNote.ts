...
export function buildCreditNoteFilename(note: CreditNotePDFData) {
...
export function downloadCreditNotePDF(note: CreditNotePDFData) {
  if (!note.items) note.items = []
  pdfMake.createPdf(buildCreditNoteDefinition(note)).download(buildCreditNoteFilename(note))
}

export function getCreditNotePDFBytes(note: CreditNotePDFData): Promise<Uint8Array> {
  if (!note.items) note.items = []
  return new Promise((resolve, reject) => {
    try {
      pdfMake.createPdf(buildCreditNoteDefinition(note)).getBuffer((buffer: any) => {
...
function deriveIsInterState(note: CreditNotePDFData): boolean {
...
function buildCreditNoteDefinition(note: CreditNotePDFData): any {
  ...
  const gridBody: TableCell[][] = [
    [
      { stack: [
        ...
        { text: note.noteNumber, fontSize: 10, margin: [0, 3, 0, 0] as [number, number, number, number] },
      ] },
      { stack: [
        { text: dateLabel, bold: true, fontSize: 10 },
        { text: formatDate(note.noteDate), fontSize: 10, margin: [0, 3, 0, 0] as [number, number, number, number] },
      ] },
    ],
  ]
  if (hasRefInv) {
    gridBody.push([
      { stack: [
        ...
        { text: note.referenceInvoice!.invoiceNumber, fontSize: 10, margin: [0, 3, 0, 0] as [number, number, number, number] },
      ] },
      { stack: [
        { text: 'Ref. Invoice Date', bold: true, fontSize: 10 },
        { text: note.referenceInvoice!.invoiceDate ? formatDate(note.referenceInvoice!.invoiceDate) : '-', fontSize: 10, margin: [0, 3, 0, 0] as [number, number, number, number] },
      ] },
    ])
  }
...
function buildBillToSection(note: CreditNotePDFData): Content {
...
function buildItemsSection(note: CreditNotePDFData, isInter: boolean, taxGroups: ReturnType<typeof getTaxGroups>): Content {
  ...
  const totalRow: TableCell[] = [
    ...
    { text: '', fillColor: GREEN },
    { text: fmtRs(note.totalAmount), bold: true, fontSize: 9, alignment: 'right' as const, fillColor: GREEN },
  ]
...
function buildFooter(note: CreditNotePDFData, logo: string | undefined, signature: string | undefined): Content {
  ...
  if (hasReason) {
    reasonAndNotesStack.push(
      { text: 'Reason', bold: true, fontSize: 9, margin: [0, 10, 0, 5] as [number, number, number, number] },
      { text: note.reason || '', fontSize: 9, lineHeight: 1.2 },
    )
  }
  if (hasNotes) {
    reasonAndNotesStack.push(
      { text: 'Notes', bold: true, fontSize: 9, margin: [0, hasReason ? 6 : 10, 0, 5] as [number, number, number, number] },
